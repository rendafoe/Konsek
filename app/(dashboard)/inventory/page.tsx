        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {groupedItems.map((groupedItem) => (
            <div
              ...
              className={`bg-card p-4 pixel-border relative group cursor-pointer transition-all hover:-translate-y-1 ${groupedItem.equippedId ? 'ring-4 ring-primary ring-offset-2 ring-offset-background' : ''}`}
            ...
            >
              ...
              {groupedItem.equippedId && (
                <div className="absolute top-2 left-2 bg-primary text-[8px] font-pixel text-primary-foreground px-1 py-0.5">EQP</div>
              )}
              {groupedItem.count > 1 && (
                <div className="absolute bottom-2 right-2 bg-muted text-[10px] font-bold text-foreground px-1.5 py-0.5 rounded-sm min-w-[20px] text-center">x{groupedItem.count}</div>
              )}
            </div>
          ))}
        </div>
