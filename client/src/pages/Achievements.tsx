...
export default function Achievements() {
  ...
  const handlePurchase = (itemId: number, itemName: string, price: number) => {
    if (!medalStatus || medalStatus.balance < price) {
      toast({
        title: "Insufficient Medals",
        description: `You need ${price} Medals to purchase this item.`,
        variant: "destructive",
      });
      return;
    }
    ...
    purchaseItem(itemId, {
      onSuccess: (data) => {
        toast({
          ...
          description: `You obtained ${itemName}!`,
        });
  ...
  // Separate mythic items from regular items
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row font-ui">
      ...
      <main className="flex-1 p-6 md:p-12 mb-20 md:mb-0 overflow-y-auto">
        <PageHeader
          title="Achievements"
          subtitle={`Collect all items from your running adventures. ${unlockedCount} / ${totalCount} unlocked`}
        />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {mythicItems.map((item) => (
                <div
                  key={item.id}
                  className={`
                    bg-card p-3 pixel-border relative transition-all duration-200 group
                    ${item.unlocked
                      ? "ring-4 ring-green-500 ring-offset-2 ring-offset-background shadow-xl shadow-green-500/40"
                  ...
                  style={{
                    background: item.unlocked
                      ? undefined
                      : "linear-gradient(135deg, rgba(255,215,0,0.1) 0%, rgba(255,105,180,0.1) 50%, rgba(138,43,226,0.1) 100%)",
                    borderImage: "linear-gradient(135deg, #ffd700, #ff69b4, #8b2be2, #ffd700) 1",
                  }}
                >
                  ...
                  <div className="aspect-square bg-gradient-to-br from-yellow-50 via-pink-50 to-purple-50 border-2 border-yellow-200 mb-2 flex items-center justify-center overflow-hidden rounded">
                    <img
                      ...
                      className={`
                        ...
                        ${!item.unlocked ? "opacity-60 grayscale-[50%] group-hover:opacity-100 group-hover:grayscale-0" : ""}
                      `}
                      onError={(e) => {
                  ...
                  {/* Item name */}
                  <h3 className="font-pixel text-[9px] truncate mb-1 text-foreground" title={item.name}>
                    {item.name}
                  </h3>

                  {/* Rarity with gradient */}
                  <p className={`text-[8px] capitalize font-bold ${rarityTextColors[item.rarity]}`}>
                    {item.rarity}
                  </p>

                  {/* Price tag or Owned badge */}
                  {item.unlocked ? (
                    <div className="mt-2 text-[10px] font-semibold text-green-600 bg-green-50 px-2 py-1 rounded text-center">
                      Owned
                    </div>
                  ) : item.price ? (
                    <button
                      onClick={() => handlePurchase(item.id, item.name, item.price!)}
                      disabled={isPurchasing && purchasingItemId === item.id}
                      className={`
                        mt-2 w-full flex items-center justify-center gap-1 text-[10px] font-bold px-2 py-1.5 rounded transition-all
                        ${medalStatus && medalStatus.balance >= item.price
                          ? "bg-gradient-to-r from-yellow-400 to-amber-500 text-white hover:brightness-110 cursor-pointer"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {regularItems.map((item) => (
            ...
            >
              ...
              {/* Item image - grayscale when locked, full color on hover */}
              <div className="aspect-square bg-background border-2 border-border mb-2 flex items-center justify-center overflow-hidden">
                <img
                  ...
                  className={`
                    w-full h-full object-contain transition-all duration-200
                    ${!item.unlocked ? "opacity-40 grayscale group-hover:opacity-100 group-hover:grayscale-0" : ""}
                  `}
                  onError={(e) => {
                    e.currentTarget.style.display = "none";
                  }}
              ...
              {/* Item name - always colored (black) */}
              <h3 className="font-pixel text-[9px] truncate mb-1 text-foreground" title={item.name}>
                {item.name}
              </h3>

              {/* Rarity - always colored */}
              <p className={`text-[8px] capitalize font-semibold ${rarityTextColors[item.rarity] || "text-gray-400"}`}>
                {item.rarity}
              </p>
